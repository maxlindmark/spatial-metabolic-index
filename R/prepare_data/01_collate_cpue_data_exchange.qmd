...
dat_haul$substrate <- terra::extract(substrate_longlat, dat_haul |> dplyr::select(lon, lat))$BALANCE_SEABED_SEDIMENT

unique(dat_haul$substrate)

factor(sort(unique(round(dat_haul$substrate))))
 
dat_haul$substrate <- round(dat_haul$substrate)

dat_haul <- dat_haul %>% mutate(substrate = ifelse(substrate == 1, "bedrock", substrate),
...
# Plot
ggplot(dat_haul, aes(lon, lat, color = substrate)) +
  geom_point()
```
