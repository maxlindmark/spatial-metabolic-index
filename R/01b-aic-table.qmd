...
aic <- read_csv(paste0(home, "/output/data_aic_01.csv")) |> 
  mutate(formula = ifelse(model == "m0", "*temp+oxy*", NA),
         ...
         formula = ifelse(model == "m4", "*\u03C6*", formula),
         formula = ifelse(model == "m5", "*bp(\u03C6)*", formula)
         ) |> 
  group_by(group) |> 
...
aict <- aic |> 
  mutate(delta_aic = round(delta_aic, digits = 1)) |> 
  mutate(delta_aic = ifelse(delta_aic == 0, "**0**", delta_aic)) |> 
  dplyr::select(group2, formula, delta_aic) |> 
  pivot_wider(names_from = formula, values_from = delta_aic) |> 
  arrange(group2)

kableExtra::kbl(aict, format = "html", escape = FALSE) |>
