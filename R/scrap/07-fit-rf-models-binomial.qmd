...
d <- read_csv(paste0(home, "/output/rf_dat_long.csv")) |> 
  ...
  dplyr::select(binom_trend, temp_slope, mean_temp, oxy_slope, mean_oxy, mean_sal, sal_slope, group, mean_biom) |> 
  mutate(mean_biom = (mean_biom - mean(mean_biom)) / sd(mean_biom), .by = group)
```
...
trend_recipe <- recipe(binom_trend ~ temp_slope + mean_temp + oxy_slope + mean_oxy + sal_slope + mean_sal + group + mean_biom,
                       data = trend)
...
ggsave(paste0(home, "/figures/supp/tuning_binom.pdf"), width = 12, height = 9, units = "cm")
...
# Save!
saveRDS(final_trend_model, paste0(home, "/output/rf_final_trend_model_binom.rds"))
```
...
collect_predictions(rf_trend_fit) |>
  ggplot(aes(binom_trend, .pred)) +
  geom_point(alpha = 0.5, color = "steelblue") +
  labs(x = "Observed", y = "Predicted") +
  geom_abline(linetype = 2, color = "tomato3") +
  NULL

ggsave(paste0(home, "/figures/supp/obs_pred_binom.pdf"), width = 11, height = 11, units = "cm")
```

Load model

```{r}
final_trend_model <- readRDS(paste0(home, "/output/rf_final_trend_model_binom.rds"))
```
...
explainer_rf <- explain_tidymodels(
    final_trend_model, 
    data = dplyr::select(trend, -binom_trend), 
    y = trend$binom_trend,
    label = "random forest"
)

# model_profile(explainer_rf,
#               variables = "mean_temp",
#               type = "accumulated")

pdp_rf <- model_profile(explainer_rf, N = NULL, 
                        variables = c("mean_temp", "mean_oxy", "mean_sal",
                                      "temp_slope", "oxy_slope", "sal_slope",
                                      "mean_biom"),
                        groups = "group"
                        )
...
p2 <- pd |> 
  ...
  ggplot(aes(x, yhat, color = group2)) +
  ...
  geom_smooth(aes(x, yhat_ct), inherit.aes = FALSE, se = FALSE,
...
ggsave(paste0(home, "/figures/vip_effects_binom.pdf"), width = 18, height = 17, units = "cm")
```
...
ggsave(paste0(home, "/figures/supp/partial_dependence_binom.pdf"), width = 18, height = 11, units = "cm")
```
...
#   data_grid(mean_temp = seq_range(mean_temp, n = 100, trim = 0.1)) |> 
#   ungroup() |> 
#   mutate(temp_vel = mean(d_new$temp_vel),
#          oxy_vel = mean(d_new$oxy_vel),
