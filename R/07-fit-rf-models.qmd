...
ggsave(paste0(home, "/figures/vip.pdf"), width = 9, height = 7, units = "cm")
...
pd <- pd |>
  ...
  mutate(lwr = quantile(x, probs = 0.05),
         ...
         .by = c(group2, vname)) |> 
  filter(x > lwr & x < upr) |> 
  mutate(yhat_ct = yhat / mean(yhat), .by = group2)

p1 <- pd |> 
  filter(var2 %in% c("Mean temperature (°C)", "Mean oxygen (ml/L)")) |> 
...
ggsave(paste0(home, "/figures/effects.pdf"), width = 18, height = 11, units = "cm")
```
